...
function makeTwo() {
	window.localStorage.setItem('string', '7,6,5');//Ваш код
	console.log('Строка "7,6,5" была записана в Local Storage.');
}

document.querySelector('.b-2').onclick = makeTwo;//добавьте слушатель события
...
function makeThree() {
	window.localStorage.setItem('person', 'name=John, age=25');//Ваш код
...
function makeFour() {
	window.localStorage.setItem('positiveValue', true);//Ваш код
...
function makeFive() {
	window.localStorage.setItem('noValue', null);//Ваш код
...
function makeEight() {
	...
	window.localStorage.setItem('objectEight', JSON.stringify(obj));//Ваш код
	console.log('Объект obj был записан в Local Storage.');
}
...
function showEleven() {
	const arrayEleven = JSON.parse(localStorage.getItem('arrayEleven'));//Ваш код
	let output = '';
...
document.querySelector('.b-13').onclick = showTen;//добавьте слушатель события
...
const showNine = () => {
	...
	if (arrayNineString) {
		const arrayNine = JSON.parse(arrayNineString);//Ваш код
...
const showEight = () => {
	...
	if (objectEightString) {
		const objectEight = JSON.parse(objectEightString);//Ваш код
...
const showSeven = () => {
	...
	if (objectSevenString) {
		const objectSeven = JSON.parse(objectSevenString);//Ваш код
...
const showSix = () => {
	...
	if (objectSixString) {
		const objectSix = JSON.parse(objectSixString);//Ваш код
...
const addToNumbers = () => {
	...
	window.localStorage.setItem('task19', numbers);//Ваш код
};
...
const removeLastElement = () => {
	if (numbers.length > 0) {
		...
		window.localStorage.setItem('task20', numbers);//Ваш код
...
const addToCart = () => {
	...
	if (product.trim() !== '') {
		let cartItems = localStorage.getItem('cartItems1');
		cartItems = cartItems ? JSON.parse(cartItems1) : []; //код из подсказки

		cartItems1.push(product);
		window.localStorage.setItem('task21', cartItems1);//Ваш код
...
document.cookie = 'user=Кот Учёный';//Ваш код
